...
const Home = () => {
    ...
    useEffect(() => {
        ...
        // Fetch thông tin mới nhất của user (để cập nhật số dư)
        fetch(`http://localhost:8080/users/${parsedUser.id}`)
            .then(res => res.json())
            .then(data => {
                setCurrentUser(data);
                // Cập nhật lại localStorage để các lần load sau cũng có data mới
    ...
    const handleTransferSubmit = (e) => {
        ...
        fetch('http://localhost:8080/transactions/transfer', {
            ...
            .catch(err => {
                ...
                alert("Đã có lỗi xảy ra khi chuyển tiền!");
            });
    };
